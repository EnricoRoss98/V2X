    ...
    if x % step_auto_test == 0:
        ...
        if prove_fissate_auto == 10:
            ...
            p = pool.apply_async(BatchLib.run,
                                 (n_port + x, n_auto, tempo_generazione, gui))
            p1 = pool.apply_async(BatchLib.run,
                                  (n_port + 1 + x, n_auto, tempo_generazione, gui))
            p2 = pool.apply_async(BatchLib.run,
                                  (n_port + 2 + x, n_auto, tempo_generazione, gui))
            p3 = pool.apply_async(BatchLib.run,
                                  (n_port + 3 + x, n_auto, tempo_generazione, gui))
            p4 = pool.apply_async(BatchLib.run,
                                  (n_port + 4 + x, n_auto, tempo_generazione, gui))
            p5 = pool.apply_async(BatchLib.run,
                                  (n_port + 5 + x, n_auto, tempo_generazione, gui))
            p6 = pool.apply_async(BatchLib.run,
                                  (n_port + 6 + x, n_auto, tempo_generazione, gui))
            p7 = pool.apply_async(BatchLib.run,
                                  (n_port + 7 + x, n_auto, tempo_generazione, gui))
            p8 = pool.apply_async(BatchLib.run,
                                  (n_port + 8 + x, n_auto, tempo_generazione, gui))
            p9 = pool.apply_async(BatchLib.run,
                                  (n_port + 9 + x, n_auto, tempo_generazione, gui))
